...
function KPI2(fk_empresa, dataAtual, hectare, setores, sensores) {
  ...
  var Kpi02selectPlantacao = `SELECT nomeFazenda, TRUNCATE(AVG(umidade), 0) AS umidade FROM vw_kpis WHERE 
        nomeFazenda = 'Recanto do Sol' AND dataCaptura LIKE '%${dataAtual}%' AND fkEmpresa = ${fk_empresa};`;

  var Kpi02selectHectare = `
            SELECT nomeFazenda, nomeHectares AS Hectare, TRUNCATE(AVG(umidade), 0) AS umidade 
  ...
  var Kpi02selectSetores = `
            SELECT nomeFazenda, nomeHectares AS Hectare, nomeSetores AS nomeSetores, TRUNCATE(AVG(umidade), 0) AS umidade 
  ...
  var Kpi02selectSensores = `
            SELECT nomeFazenda, nomeHectares AS Hectare, nomeSetores AS nomeSetores, codSensor AS codSensor, TRUNCATE(AVG(umidade), 0) AS umidade 
...
function KPI3(fk_empresa, hectare, setores, sensores, dataAtual, dataMenos30) {
  var Kpi03selectPlantacao = `
   SELECT nomeFazenda, TRUNCATE(AVG(umidade), 0) AS umidade FROM vw_kpis WHERE fkEmpresa = ${fk_empresa} AND
  nomeFazenda = 'Recanto do Sol' 
  AND dataCaptura BETWEEN '${dataMenos30}' AND '${dataAtual}';
  `;

  var Kpi03selectHectare = `
    SELECT nomeHectares, TRUNCATE(AVG(umidade), 0) AS umidade FROM vw_kpis WHERE nomeHectares = 'Hectare ${hectare}' AND dataCaptura BETWEEN '${dataMenos30}' AND '${dataAtual}';
  `;

  var Kpi03selectSetores = `
 SELECT nomeSetores, TRUNCATE(AVG(umidade), 0) AS umidade
FROM vw_kpis
...
GROUP BY nomeSetores;
  ...
  var Kpi03selectSensores = `SELECT ROUND(AVG(umidade), 0) AS umidade FROM vw_kpis WHERE nomeFazenda = 'Recanto do Sol'AND nomeHectares = 'Hectare ${hectare}'AND nomeSetores = 'Setor ${hectare}-${setores}'AND codSensor = ${sensores}
  AND dataCaptura BETWEEN '${dataMenos30}' AND '${dataAtual}';
  `;
...
function Grafico1(fk_empresa, hectare, setores, sensores, dataAtual) {
  var Grafico1Hectare = `SELECT TRUNCATE(AVG(umidade), 0) AS umidade, date_format(dataCaptura, '%H:%i') AS dataCaptura 
...
ORDER BY dataCaptura;`;
  ...
  var Grafico1Plantacao = `SELECT TRUNCATE(AVG(umidade), 0 ) AS umidade, date_format(dataCaptura, '%H:%i') AS dataCaptura 
FROM vw_kpis WHERE fkEmpresa = ${fk_empresa} AND nomeFazenda = 'Recanto do Sol' AND DAY(dataCaptura) = (SELECT DAY(CURRENT_DATE))
AND (dataCaptura LIKE '%00:00:00%' OR dataCaptura LIKE '%04:00:00%' OR dataCaptura LIKE '%08:00:00%'
OR dataCaptura LIKE '%12:00:00%' OR dataCaptura LIKE '%16:00:00%' OR dataCaptura LIKE '%20:00:00%')
GROUP BY nomeFazenda, dataCaptura
ORDER BY dataCaptura;`;
  ...
  var Grafico1Setor = `SELECT TRUNCATE(AVG(umidade), 0) AS umidade, date_format(dataCaptura, '%H:%i') AS dataCaptura 
FROM vw_kpis WHERE fkEmpresa = ${fk_empresa} AND nomeFazenda = 'Recanto do Sol' AND DAY(dataCaptura) = (SELECT DAY(CURRENT_DATE))
...
ORDER BY dataCaptura;`;
  ...
  var Grafico1Sensor = `SELECT umidade AS umidade, date_format(dataCaptura, '%H:%i') AS dataCaptura 
FROM vw_kpis WHERE fkEmpresa = ${fk_empresa} AND nomeFazenda = 'Recanto do Sol' AND DAY(dataCaptura) = (SELECT DAY(CURRENT_DATE))
